#!/usr/bin/perl
# commandline_SEAR.pl is the command line version of the SEAR website and script that identifies full length antimicrobial resistance genes in fastq sequencing data. It takes the arguments passed from the command line and user specified files and then uses several pieces of open source software.
use warnings;
use strict;
use Sear;
...
my $temp_files_directory = "$SEAR_DATA_directory/temp_files";
my $temp_file_name;
...
{
    print "\t$in_file\n";
    $temp_file_name = $start_timestamp . "_" . basename($in_file);
    $temp_file_name =~ s/[^$safe_filename_characters]/_unsafechar_/g;
    $temp_files_directory .= "_$temp_file_name";
...
my $opt_results = "$SEAR_DATA_directory/RESULTS/SEAR_RESULTS_$temp_file_name";
my $multifastq = "consensus_seqs.fq";
system ( "mkdir $temp_files_directory" ) == 0 or die ( "Can't make temporary files directory: $?.\n" );
...
while ($fastq_split = readdir(TEMPDIR))
{
    ...
    print "$input_counter, ";
    my $fastq_command = "usearch -fastq_filter $temp_files_directory/$fastq_split -fastq_minlen $opt_length -fastaout $temp_files_directory/$input_counter.fasta >> $log 2>&1";
...
while ($usearch_infile = readdir(INDIR))
{
    ...
    my $usearch_command = "usearch -usearch_global $temp_files_directory/$usearch_infile -db $temp_files_directory/$reference_args_udb -id $opt_clustering_identity -strand both -maxhits 1 -threads $opt_threads -uc $temp_files_directory/$counter.usearchfile.uc -matched $temp_files_directory/$counter.matchedreads >> $log 2>&1";
    print "$counter, ";
...
if (%abundance_values)
{
    ...
    #   create html files
    my $results_redirect = "./SEAR_RESULTS_" . $temp_file_name . "/html_files/";
    my $html_results_page = `cat $SEAR_DATA_directory/references/html_templates/RESULTS_template.html`;
    my $joined_input_files = join (", ",@parsed_input_filenames);
    my $input_files_line = "<p class=\"lead\">Input files:</p>";
    my $input_files_line_entry = "<p class=\"lead\">Input files:   $joined_input_files</p>";
    my $sear_settings_line = "<td>...end of SEAR settings...</td>";
                if ($html_results_page =~ m/\Q$input_files_line\E/)
                {
                    $html_results_page =~ s/\Q$input_files_line\E/$input_files_line_entry/;
                }
    ...
    my $flat_file_entry = "href=\"" . "./SEAR_RESULTS_" . $temp_file_name . "/flat_file.tar\"";
    ...
    open RESULTS_PAGE_OUTFILE, ">$SEAR_DATA_directory/RESULTS/SEAR_RESULTS_$temp_file_name.html" or die "Can't generate results page: $!";
    ...
    print "SEAR Finished\n";
    print "################################################\n\n";
    my $results_link = "$SEAR_DATA_directory/RESULTS/SEAR_RESULTS_" . $temp_file_name . ".html\n";
    print "Results can be found at: $results_link";
    
    my $SEAR_finish_time = Time::HiRes::tv_interval($start_time);
    print "Elapsed time:\t$SEAR_finish_time\n\n";
    system ("mv $log $opt_results && rm -r $temp_files_directory") == 0 or die "Can't move log file and delete temp file folder from server: $!";
    print "A log for this SEAR run has been made at: $opt_results\n\n";
    exit(0);
}
...
else
{
    ...
    my $SEAR_finish_time = Time::HiRes::tv_interval($start_time);
    print "Elapsed time:\t$SEAR_finish_time\n\n";
    system ("mv $log $opt_results && rm -r $temp_files_directory && rm $opt_results/$multifastq") == 0 or die "Can't move log file and delete temp file folder from server: $!";
    print "A log for this SEAR run has been made at: $opt_results\n\n";
    exit(0);
}
