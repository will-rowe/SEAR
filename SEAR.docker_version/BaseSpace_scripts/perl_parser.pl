...
my $filter_reads = $decoded_json->{Properties}->{Items}->[9]->{Content};
...
# Initiate SEAR
my $SEAR = "/usr/bin/perl /data/scratch/SEAR/basespace_SEAR.pl $input_file_list -lc $length_cutoff -qc $QS_cutoff -cc $annotation_stringency -ci $clustering_stringency -dd /data/scratch/SEAR/SEAR_DATA -f $filter_reads -t 32 -po $project_output";
